...
final class RealmAccessorTest: XCTestCase {
    ...
    func testReadWithFilter() async throws {
        ...
        guard let result = await realm.read(type: TrainingTagEntity.self, where: { $0.tagName == "testData1" }).first else {
    ...
    func testReadAll() async throws {
        ...
        let result = await realm.read(type: TrainingTagEntity.self)
    ...
    func testUpdate() async throws {
        ...
        guard let result = await realm.read(type: TrainingTagEntity.self, where: { $0.id == expectedData.id }).first else {
    ...
    func testDelete() async throws {
        ...
        guard await realm.delete(type: TrainingTagEntity.self, where: { $0.id == filterId }) else {
            
            XCTFail("Fail delete")
            return
        }
        
        let results = await realm.read(type: TrainingTagEntity.self)
...
private extension RealmAccessorTest {
    ...
    func deleteTestData(deleteTarget: UUID) async {
        ...
        let realm = await RealmAccessor()
        guard await realm.delete(type: TrainingTagEntity.self, where: { $0.id == deleteTarget }) else {
