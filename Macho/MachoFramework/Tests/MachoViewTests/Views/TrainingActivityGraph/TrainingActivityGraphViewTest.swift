...
final class TrainingActivityGraphViewTest: XCTestCase {
    ...
    func test_画面表示時に保存しているフィルターを画面に反映してフィルターに該当するアクティビティ情報を表示する() async throws {
        ...
        let mockTrainingTypeClient = await TrainingTypeClient.getMockClient(
            realm: mockRealm,
            initialValue: [.abs, .benchPress]
        )
        ...
        await testStore.receive(\.startObserve.observeDiaryList)
        await testStore.receive(\.didReceiveDiaryData) {
            
            $0.activityResultList = self.createExpectedActivityResults(
                [expectedFetchDiaryData1, expectedFetchDiaryData2],
                initialStartPeriod,
                initialPeriod,
                []
            )
            $0.calendar.decorationSources = try self.createExpectedDecorationDic(expectedFetchDiaryData1)
            $0.calendar.decorationSources += try self.createExpectedDecorationDic(expectedFetchDiaryData2)
        }
        
        await testStore.receive(\.startObserve.observeTrainingType)
        
        await testStore.receive(\.didReceiveTrainingTypeList) {
            
            $0.targetTrainingTypeList = initialSelectedTrainingTypeList
        }
        await testStore.receive(\.didReceiveDiaryData) {
